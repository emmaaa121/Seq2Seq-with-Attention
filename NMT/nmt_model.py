...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(embed_size, embed_size, kernel_size = 2, padding = 'same') #PDF before step (1)
        self.encoder = nn.LSTM(embed_size, hidden_size, bidirectional=True)                        #PDF step (1)(2)
        self.decoder = nn.LSTMCell(embed_size+hidden_size, hidden_size)                            #PDF step (5) y_t concat with o_t-1
        self.h_project = nn.Linear(2 * hidden_size, hidden_size, bias=False)                       #PDF step (3) all linear layers train weights and bias(no biase here), weight size is output_size * input_size
        self.c_project = nn.Linear(2 * hidden_size, hidden_size, bias=False)                       #PDF step (4) 
        self.att_projection = nn.Linear(2 * hidden_size, hidden_size, bias=False)                  #PDF step (7) 
        self.combined_output_projection = nn.Linear(3 * hidden_size, hidden_size, bias=False)      #PDF step (10) 
        self.target_vocab_projection = nn.Linear(hidden_size, len(vocab.tgt), bias=False)          #PDF step (13) 
        ...
        #CLASStorch.nn.LSTMCell(input_size, hidden_size, bias=True, device=None, dtype=None)
        ###     Linear Layer:
        ###         https://pytorch.org/docs/stable/nn.html#torch.nn.Linear
        #LINEAR CLASStorch.nn.Linear(in_features, out_features, bias=True, device=None, dtype=None)
        ###     Dropout Layer:
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        @param source_padded (Tensor): Tensor of padded source sentences with shape (src_len, b), where                 #src_len as row and b as columns? --> after embedding (src_len, b, e) 
                                        b = batch_size, src_len = maximum source sentence length. Note that
                                       these have already been sorted in order of longest to shortest sentence.
        @param source_lengths (List[int]): List of actual lengths for each of the source sentences in the batch
        @returns enc_hiddens (Tensor): Tensor of hidden units with shape (b, src_len, h*2), where                       #b as numbers of array, src_len and h as column?
        ...
        ###         https://pytorch.org/docs/stable/generated/torch.Tensor.reshape.html
                #scource_padded  (src_len, b)
        X = self.model_embeddings.source(source_padded)    #X: (src_len, b, e)
        X = self.post_embed_cnn(torch.permute(X, (1, 2, 0))).torch.permute((2, 0, 1)) #X: (src_len, b, e) -->  (b, e, src_len) --> (src_len, b, e)
        X = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encode(X)
        enc_hiddens, length = pad_packed_sequence(enc_hiddens)
        ...
        last_hidden = self.h_project(last_hidden)
        last_cell = torch.cat((last_cell[0], last_cell[1]), 1) #(2, b, h). --> (b, 2*h)
        last_cell  = self.c_project(last_cell)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        batch_size = enc_hiddens.size(0) #returns the batch size, which is the number of sequences being processed in parallel.
        ...
        Y = self.model_embeddings.tgt(target_padded)    #Y: (src_len, b, e)
        for Y_t in torch.split(Y, 1): #(1, b, e)    torch.split(tensor, split_size, dim=0)
            Y_t = torch.squeeze(Y_t)    #(b, e)     returns a tensor with all size 1 removed.
            Ybar_t = torch.cat((Y_t, o_prev), 1)  #(b, e+h)  torch.cat((x, x, x), 0/1)
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t
        combined_outputs = torch.stack(combined_outputs, dim=0) #torch.stack(tensors, dim=0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decode(Ybar_t, dec_state) # (1, b, h)
        dec_hidden, dec_cell = dec_state[0], dec_state[1]# (1, b, h)
        e_t = torch.bmm(
            enc_hiddens_proj,               #(b, src_len, h)
